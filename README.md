...
Windows 用の画像生成AIにて作成された画像/動画ビューワー。
...
- 動画 :  
  MP4 / WebM / MKV（再生可否は中のコーデックにも依存します。再生できない動画は
  プレーヤーから「既定のアプリで開く」で外部再生できます。一覧のサムネイルはエクスプローラーと
  同じ範囲の形式で表示されます）
...
├── VideoWindow.xaml(.cs)       動画プレーヤーウィンドウ
...
- 生成 AI 画像・動画のメタデータ表示は、各フォーマット（NovelAI / ComfyUI / C2PA など）の仕様に依存します。フォーマット側の変更や情報が埋め込まれていないファイルでは表示できないことがあります。
- ご利用は自己責任でお願いします。
